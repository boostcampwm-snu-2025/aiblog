...
app.get(
  '/api/github/:owner/:repo/recent',
  async (req: Request<{ owner: string; repo: string }>, res: Response) => {
    try {
...
app.get('/api/posts', async (_req: Request, res: Response) => {
  res.json(await PostsRepo.list());
});

app.get('/api/posts/:id', async (req: Request<{ id: string }>, res: Response) => {
  const p = await PostsRepo.get(req.params.id);
...
app.post('/api/posts', async (req: Request, res: Response) => {
  ...
  const p = await PostsRepo.create({ title, markdown, tags });
  res.status(201).json(p);
});

app.put('/api/posts/:id', async (req: Request<{ id: string }>, res: Response) => {
  const p = await PostsRepo.update(req.params.id, req.body || {});
  if (!p) return res.status(404).end();
  res.json(p);
});

app.delete('/api/posts/:id', async (req: Request<{ id: string }>, res: Response) => {
